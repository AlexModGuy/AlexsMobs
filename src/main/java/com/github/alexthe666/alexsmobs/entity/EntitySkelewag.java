...
public class EntitySkelewag extends Monster implements IAnimatedEntity {
    ...
    public static boolean canSkelewagSpawn(EntityType<EntitySkelewag> type, ServerLevelAccessor levelAccessor, MobSpawnType p_32352_, BlockPos below, Random random) {
        if (!levelAccessor.getFluidState(below.below()).is(FluidTags.WATER)) {
            ...
            boolean flag = levelAccessor.getDifficulty() != Difficulty.PEACEFUL && isDarkEnoughToSpawn(levelAccessor, below, random) && (p_32352_ == MobSpawnType.SPAWNER || random.nextInt(3) == 0 && levelAccessor.getFluidState(below).is(FluidTags.WATER));
