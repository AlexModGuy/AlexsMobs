...
public class WarpedToadAIRandomSwimming extends RandomWalkingGoal {
    public WarpedToadAIRandomSwimming(EntityWarpedToad creature, double speed, int chance) {
        super(creature, speed, chance, false);
    }

    public boolean shouldExecute() {
        if (this.creature.isBeingRidden() || ((EntityWarpedToad)creature).isSitting() || creature.getAttackTarget() != null || !this.creature.isInWater() && !this.creature.isInLava() && this.creature instanceof ISemiAquatic && !((ISemiAquatic) this.creature).shouldEnterWater()) {
            return false;
        } else {
            if (!this.mustUpdate) {
