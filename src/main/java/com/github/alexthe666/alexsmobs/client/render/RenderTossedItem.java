...
public class RenderTossedItem  extends EntityRenderer<EntityTossedItem> {
    ...
    public RenderTossedItem(EntityRenderDispatcher renderManager) {
        super(renderManager);
    }
    ...
    public void render(EntityTossedItem entityIn, float entityYaw, float partialTicks, PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn) {
        ...
        if(entityIn.isDart()){
            ...
            matrixStackIn.mulPose(Vector3f.YP.rotationDegrees(Mth.lerp(partialTicks, entityIn.yRotO, entityIn.yRot) - 180F));
            matrixStackIn.pushPose();
            ...
            Minecraft.getInstance().getItemRenderer().renderStatic(entityIn.getItem(), ItemTransforms.TransformType.GROUND, packedLightIn, OverlayTexture.NO_OVERLAY, matrixStackIn, bufferIn);
        }
        matrixStackIn.popPose();
    }
