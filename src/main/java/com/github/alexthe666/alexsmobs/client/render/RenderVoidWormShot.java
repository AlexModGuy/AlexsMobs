...
public class RenderVoidWormShot extends EntityRenderer<EntityVoidWormShot> {
    private static final ResourceLocation TEXTURE = ResourceLocation
            .parse("alexsmobs:textures/entity/void_worm/void_worm_shot.png");
    ...
    @Override
    public void render(EntityVoidWormShot entityIn, float entityYaw, float partialTicks, PoseStack matrixStackIn,
        ...
        float home = (entityIn.prevStopHomingProgress
                + (entityIn.getStopHomingProgress() - entityIn.prevStopHomingProgress) * partialTicks)
                / EntityVoidWormShot.HOME_FOR;
        float colorize = home;
        matrixStackIn.translate(0, -1.5F, 0);
        VertexConsumer ivertexbuilder = bufferIn.getBuffer(AMRenderTypes.getFullBright(getTextureLocation(entityIn)));
        MODEL.renderToBuffer(matrixStackIn, ivertexbuilder, 210, NO_OVERLAY,
                AMColorUtil.packColor(Math.max(colorize, 0.2F), Math.max(colorize, 0.2F), 1.0F, 1.0F));
