...
public class LavaVisionFluidRenderer extends LiquidBlockRenderer {
    ...
    private static boolean isFaceOccludedByNeighbor(BlockGetter p_239283_0_, BlockPos p_239283_1_,
            Direction p_239283_2_, float p_239283_3_) {
    ...
    private static boolean isFaceOccludedByState(BlockGetter p_239284_0_, Direction p_239284_1_, float p_239284_2_,
            BlockPos p_239284_3_, BlockState p_239284_4_) {
    ...
    public void tesselate(BlockAndTintGetter lightReaderIn, BlockPos posIn, VertexConsumer vertexBuilderIn,
        ...
        try {
            if (fluidStateIn.is(FluidTags.LAVA)) {
                TextureAtlasSprite[] atextureatlassprite = net.neoforged.neoforge.client.textures.FluidSpriteCache
                        .getFluidSprites(lightReaderIn, posIn, fluidStateIn);
                final int i = net.neoforged.neoforge.client.extensions.common.IClientFluidTypeExtensions
                        .of(fluidStateIn).getTintColor(fluidStateIn, lightReaderIn, posIn);
                ...
                boolean flag2 = shouldRenderFace(lightReaderIn, posIn, fluidStateIn, blockstateIn, Direction.DOWN,
                        fluidstate)
                        && !isFaceOccludedByNeighborVanilla(lightReaderIn, posIn, Direction.DOWN, 0.8888889F,
                                blockstate);
                boolean flag3 = shouldRenderFace(lightReaderIn, posIn, fluidStateIn, blockstateIn, Direction.NORTH,
                        fluidstate2);
                boolean flag4 = shouldRenderFace(lightReaderIn, posIn, fluidStateIn, blockstateIn, Direction.SOUTH,
                        fluidstate3);
                boolean flag5 = shouldRenderFace(lightReaderIn, posIn, fluidStateIn, blockstateIn, Direction.WEST,
                        fluidstate4);
                boolean flag6 = shouldRenderFace(lightReaderIn, posIn, fluidStateIn, blockstateIn, Direction.EAST,
                        fluidstate5);
                if (!flag1 && !flag2 && !flag6 && !flag5 && !flag3 && !flag4) {
                    ...
                    if (f11 >= 1.0F) {
                        ...
                        f7 = this.calculateAverageHeight(lightReaderIn, fluid, f11, f12, f14,
                                posIn.relative(Direction.NORTH).relative(Direction.EAST));
                        f8 = this.calculateAverageHeight(lightReaderIn, fluid, f11, f12, f15,
                                posIn.relative(Direction.NORTH).relative(Direction.WEST));
                        f9 = this.calculateAverageHeight(lightReaderIn, fluid, f11, f13, f14,
                                posIn.relative(Direction.SOUTH).relative(Direction.EAST));
                        f10 = this.calculateAverageHeight(lightReaderIn, fluid, f11, f13, f15,
                                posIn.relative(Direction.SOUTH).relative(Direction.WEST));
                    }
                    ...
                    if (flag1 && !isFaceOccludedByNeighborVanilla(lightReaderIn, posIn, Direction.UP,
                            Math.min(Math.min(f8, f10), Math.min(f9, f7)), blockstate1)) {
                        f8 -= 0.001F;
                        f10 -= 0.001F;
                        f9 -= 0.001F;
                        f7 -= 0.001F;
                        ...
                        float f51 = (float) atextureatlassprite[0].contents().width()
                                / (atextureatlassprite[0].getU1() - atextureatlassprite[0].getU0());
                        float f52 = (float) atextureatlassprite[0].contents().height()
                                / (atextureatlassprite[0].getV1() - atextureatlassprite[0].getV0());
                        ...
                        this.vertexVanilla(vertexBuilderIn, d1 + 0.0D, d2 + (double) f8, d0 + 0.0D, f30, f31, f32,
                                alpha, f18, f22, j);
                        this.vertexVanilla(vertexBuilderIn, d1 + 0.0D, d2 + (double) f10, d0 + 1.0D, f30, f31, f32,
                                alpha, f19, f23, j);
                        this.vertexVanilla(vertexBuilderIn, d1 + 1.0D, d2 + (double) f9, d0 + 1.0D, f30, f31, f32,
                                alpha, f20, f24, j);
                        this.vertexVanilla(vertexBuilderIn, d1 + 1.0D, d2 + (double) f7, d0 + 0.0D, f30, f31, f32,
                                alpha, f21, f25, j);
                    ...
                    if (flag2) {
                        ...
                        this.vertexVanilla(vertexBuilderIn, d1, d2 + (double) f17, d0 + 1.0D, f46, f47, f48, alpha, f40,
                                f43, l);
                        this.vertexVanilla(vertexBuilderIn, d1, d2 + (double) f17, d0, f46, f47, f48, alpha, f40, f42,
                                l);
                        this.vertexVanilla(vertexBuilderIn, d1 + 1.0D, d2 + (double) f17, d0, f46, f47, f48, alpha, f41,
                                f42, l);
                        this.vertexVanilla(vertexBuilderIn, d1 + 1.0D, d2 + (double) f17, d0 + 1.0D, f46, f47, f48,
                                alpha, f41, f43, l);
                    }
                    ...
                    for (Direction direction : Direction.Plane.HORIZONTAL) {
                        ...
                        if (flag8 && !isFaceOccludedByNeighborVanilla(lightReaderIn, posIn, direction,
                                Math.max(f44, f45), lightReaderIn.getBlockState(posIn.relative(direction)))) {
                            BlockPos blockpos = posIn.relative(direction);
                            TextureAtlasSprite textureatlassprite2 = atextureatlassprite[1];
                            if (atextureatlassprite[2] != null) {
                                if (lightReaderIn.getBlockState(blockpos).shouldDisplayFluidOverlay(lightReaderIn,
                                        blockpos, fluidStateIn)) {
                            ...
                            this.vertexVanilla(vertexBuilderIn, d3, d2 + (double) f44, d4, f37, f38, f39, alpha, f54,
                                    f33, k);
                            this.vertexVanilla(vertexBuilderIn, d5, d2 + (double) f45, d6, f37, f38, f39, alpha, f55,
                                    f34, k);
                            this.vertexVanilla(vertexBuilderIn, d5, d2 + (double) f17, d6, f37, f38, f39, alpha, f55,
                                    f35, k);
                            ...
                            if (false) {
                                ...
                                this.vertexVanilla(vertexBuilderIn, d5, d2 + (double) f17, d6, f37, f38, f39, alpha,
                                        f55, f35, k);
                                this.vertexVanilla(vertexBuilderIn, d5, d2 + (double) f45, d6, f37, f38, f39, alpha,
                                        f55, f34, k);
                                this.vertexVanilla(vertexBuilderIn, d3, d2 + (double) f44, d4, f37, f38, f39, alpha,
                                        f54, f33, k);
                            }
                        }
                    }
    ...
    private static boolean isFaceOccludedByStateVanilla(BlockGetter p_110979_, Direction p_110980_, float p_110981_,
            BlockPos p_110982_, BlockState p_110983_) {
    ...
    private static boolean isFaceOccludedByNeighborVanilla(BlockGetter p_203180_, BlockPos p_203181_,
            Direction p_203182_, float p_203183_, BlockState p_203184_) {
        return isFaceOccludedByStateVanilla(p_203180_, p_203182_, p_203183_, p_203181_.relative(p_203182_), p_203184_);
    }

    private static boolean isFaceOccludedBySelfVanilla(BlockGetter p_110960_, BlockPos p_110961_, BlockState p_110962_,
            Direction p_110963_) {
        return isFaceOccludedByStateVanilla(p_110960_, p_110963_.getOpposite(), 1.0F, p_110961_, p_110962_);
    }

    private float calculateAverageHeight(BlockAndTintGetter p_203150_, Fluid p_203151_, float p_203152_,
            float p_203153_, float p_203154_, BlockPos p_203155_) {
    ...
    private float getHeight(BlockAndTintGetter p_203161_, Fluid p_203162_, BlockPos p_203163_, BlockState p_203164_,
            FluidState p_203165_) {
