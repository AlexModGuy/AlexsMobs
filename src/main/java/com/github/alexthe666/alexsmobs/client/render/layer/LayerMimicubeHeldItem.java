...
public class LayerMimicubeHeldItem extends RenderLayer<EntityMimicube, ModelMimicube> {
    ...
    public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, EntityMimicube entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        if (!itemRight.isEmpty()) {
            ...
            Minecraft.getInstance().getItemRenderer().renderStatic(itemRight, ItemTransforms.TransformType.THIRD_PERSON_RIGHT_HAND, rightSwap > 0 ? (int) (-100 * rightSwap) : packedLightIn, LivingEntityRenderer.getOverlayCoords(entitylivingbaseIn, 0.0F), matrixStackIn, bufferIn);
        ...
        if (!itemLeft.isEmpty()) {
            ...
            Minecraft.getInstance().getItemRenderer().renderStatic(itemLeft, ItemTransforms.TransformType.THIRD_PERSON_RIGHT_HAND, leftSwap > 0 ? (int) (-100 * leftSwap) : packedLightIn, LivingEntityRenderer.getOverlayCoords(entitylivingbaseIn, 0.0F), matrixStackIn, bufferIn);
            matrixStackIn.popPose();
        }
    }
