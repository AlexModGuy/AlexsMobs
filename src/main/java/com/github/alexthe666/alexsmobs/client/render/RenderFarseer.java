...
public class RenderFarseer extends MobRenderer<EntityFarseer, ModelFarseer> {
    private static final ResourceLocation TEXTURE = ResourceLocation
            .parse("alexsmobs:textures/entity/farseer/farseer.png");
    private static final ResourceLocation TEXTURE_ANGRY = ResourceLocation
            .parse("alexsmobs:textures/entity/farseer/farseer_angry.png");
    private static final ResourceLocation TEXTURE_CLAWS = ResourceLocation
            .parse("alexsmobs:textures/entity/farseer/farseer_claws.png");
    private static final ResourceLocation TEXTURE_EYE = ResourceLocation
            .parse("alexsmobs:textures/entity/farseer/farseer_eye.png");
    private static final ResourceLocation TEXTURE_SCARS = ResourceLocation
            .parse("alexsmobs:textures/entity/farseer/farseer_scars.png");
    ...
    public void render(EntityFarseer entityIn, float entityYaw, float partialTicks, PoseStack matrixStackIn,
        ...
        if (entityIn.getAnimation() == EntityFarseer.ANIMATION_EMERGE) {
            ...
            // Render using just the portal texture without the static overlay effect
            VertexConsumer portalStatic = bufferIn.getBuffer(RenderType.entityTranslucent(PORTAL_TEXTURES[portalTexture]));
        ...
        if (entityIn.hasLaser() && laserTarget != null && !laserTarget.isRemoved()) {
            float laserProgress = (entityIn.prevLaserLvl
                    + (entityIn.getLaserAttackLvl() - entityIn.prevLaserLvl) * partialTicks)
                    / (float) EntityFarseer.LASER_ATTACK_DURATION;
            float laserHeight = entityIn.getEyeHeight();
            // float angryProgress = entityIn.prevAngryProgress + (entityIn.angryProgress -
            // entityIn.prevAngryProgress) * partialTicks;
            Vec3 angryShake = Vec3.ZERO;
            double d0 = Mth.lerp(partialTicks, laserTarget.xo, laserTarget.getX())
                    - Mth.lerp(partialTicks, entityIn.xo, entityIn.getX()) - angryShake.x;
            double d1 = Mth.lerp(partialTicks, laserTarget.yo, laserTarget.getY()) + laserTarget.getEyeHeight()
                    - Mth.lerp(partialTicks, entityIn.yo, entityIn.getY()) - angryShake.y - laserHeight;
            double d2 = Mth.lerp(partialTicks, laserTarget.zo, laserTarget.getZ())
                    - Mth.lerp(partialTicks, entityIn.zo, entityIn.getZ()) - angryShake.z;
            ...
            Matrix3f matrix3f = posestack$pose.normal();
            int j = 255;
            // apparently its static? should be moving so has to be moved manually through
            // UV
            long systemTime = Util.getMillis() * 7L;
            float u = (float) (systemTime % 30000L) / 30000.0F;
            float v = (float) Math.floor((systemTime % 3000L) / 3000.0F * 4.0F) * 0.25F
                    + (float) Math.sin(systemTime / 30000F) * 0.05F
    ...
    private void renderFarseerModel(PoseStack matrixStackIn, MultiBufferSource source, RenderType defRenderType,
        ...
        if (hurt > 0) {
            ...
            VertexConsumer staticyScars = source.getBuffer(RenderType.entityTranslucent(TEXTURE_SCARS));
        ...
        Vec3 colorOffset = entityIn.getLatencyOffsetVec(10, partialTicks).scale(-0.2F)
                .add(entityIn.angryShakeVec.scale(0.3F));
        ...
        float angryProgress = entityIn.prevAngryProgress
                + (entityIn.angryProgress - entityIn.prevAngryProgress) * partialTicks;
        float afterimageAlpha1 = defAlpha * Math
                .max(((float) Math.sin((entityIn.tickCount + partialTicks) * 0.2F) + 1F) * 0.3F, angryProgress * 0.2F);
        float afterimageAlpha2 = defAlpha * Math
                .max(((float) Math.cos((entityIn.tickCount + partialTicks) * 0.2F) + 1F) * 0.3F, angryProgress * 0.2F);
        ...
        AFTERIMAGE_MODEL.renderToBuffer(matrixStackIn, source.getBuffer(afterimage), 240, overlayColors,
                AMColorUtil.packColor(1.0F, 0F, 0F, afterimageAlpha1));
        matrixStackIn.popPose();
        matrixStackIn.pushPose();
        matrixStackIn.translate(blueOffset.x, blueOffset.y, blueOffset.z);
        AFTERIMAGE_MODEL.renderToBuffer(matrixStackIn, source.getBuffer(afterimage), 240, overlayColors,
                AMColorUtil.packColor(0F, 0F, 1.0F, afterimageAlpha2));
        matrixStackIn.popPose();
        matrixStackIn.popPose();
        AFTERIMAGE_MODEL.eye.showModel = true;
    }

    private static void laserOriginVertex(VertexConsumer p_114220_, Matrix4f p_114221_, Matrix3f p_114092_,
            int p_114222_, float xOffset, float yOffset) {
        org.joml.Vector3f normal = new org.joml.Vector3f(0.0F, 1.0F, 0.0F);
        normal.mul(p_114092_);
        org.joml.Vector4f pos = new org.joml.Vector4f(0.0F, 0.0F, 0.0F, 1.0F);
        pos.mul(p_114221_);
        p_114220_.addVertex(pos.x, pos.y, pos.z).setColor(255, 255, 255, 255).setUv(xOffset + 0.5F, yOffset)
                .setOverlay(NO_OVERLAY).setLight(240).setNormal(normal.x, normal.y, normal.z);
    }

    private static void laserLeftCornerVertex(VertexConsumer p_114215_, Matrix4f p_114216_, Matrix3f p_114092_,
            float p_114217_, float p_114218_, float xOffset, float yOffset) {
        org.joml.Vector3f normal = new org.joml.Vector3f(0.0F, -1.0F, 0.0F);
        normal.mul(p_114092_);
        org.joml.Vector4f pos = new org.joml.Vector4f(-HALF_SQRT_3 * p_114218_, p_114217_, 0.0F, 1.0F);
        pos.mul(p_114216_);
        p_114215_.addVertex(pos.x, pos.y, pos.z).setColor(255, 255, 255, 0).setUv(xOffset, yOffset + 1)
                .setOverlay(NO_OVERLAY)
                .setLight(240).setNormal(normal.x, normal.y, normal.z);
    }

    private static void laserRightCornerVertex(VertexConsumer p_114224_, Matrix4f p_114225_, Matrix3f p_114092_,
            float p_114226_, float p_114227_, float xOffset, float yOffset) {
        org.joml.Vector3f normal = new org.joml.Vector3f(0.0F, -1.0F, 0.0F);
        normal.mul(p_114092_);
        org.joml.Vector4f pos = new org.joml.Vector4f(HALF_SQRT_3 * p_114227_, p_114226_, 0.0F, 1.0F);
        pos.mul(p_114225_);
        p_114224_.addVertex(pos.x, pos.y, pos.z).setColor(255, 255, 255, 0).setUv(xOffset + 1, yOffset + 1)
                .setOverlay(NO_OVERLAY).setLight(240).setNormal(normal.x, normal.y, normal.z);
    }

    private static void portalVertex(VertexConsumer p_114090_, Matrix4f p_114091_, Matrix3f p_114092_, int p_114093_,
            float p_114094_, int p_114095_, int p_114096_, int p_114097_, float alpha) {
        org.joml.Vector3f normal = new org.joml.Vector3f(0.0F, -1.0F, 0.0F);
        normal.mul(p_114092_);
        org.joml.Vector4f pos = new org.joml.Vector4f(p_114094_ - 0.5F, (float) p_114095_ - 0.25F, 0.0F, 1.0F);
        pos.mul(p_114091_);
        p_114090_.addVertex(pos.x, pos.y, pos.z).setColor(1F, 1F, 1F, alpha)
                .setUv((float) p_114096_, (float) p_114097_).setOverlay(NO_OVERLAY).setLight(240)
                .setNormal(normal.x, normal.y, normal.z);
    }

    @Override
    protected void setupRotations(EntityFarseer farseer, PoseStack matrixStackIn, float f1, float f2, float f3, float f4) {
        float invCameraAmount = 1F
                - farseer.getFacingCameraAmount(Minecraft.getInstance().getTimer().getGameTimeDeltaPartialTick(false));
    ...
    class LayerOverlay extends RenderLayer<EntityFarseer, ModelFarseer> {
        ...
        public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn,
                EntityFarseer entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks,
