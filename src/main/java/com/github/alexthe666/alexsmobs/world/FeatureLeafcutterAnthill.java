...
public class FeatureLeafcutterAnthill extends Feature<NoneFeatureConfiguration> {
    ...
    public boolean place(WorldGenLevel worldIn, ChunkGenerator p_230362_3_, Random rand, BlockPos pos, NoneFeatureConfiguration p_230362_6_) {
        ...
        int z = 8;
        int x = 8;
        int y = worldIn.getHeight(Heightmap.Types.WORLD_SURFACE_WG, pos.getX() + x, pos.getZ() + z);
        ...
        if (worldIn.getBlockState(heightPos.above(outOfGround + 1)).getBlock() != AMBlockRegistry.LEAFCUTTER_ANTHILL && worldIn.getBlockState(heightPos.above(outOfGround - 1)).getBlock() != AMBlockRegistry.LEAFCUTTER_ANTHILL) {
            ...
            BlockEntity tileentity = worldIn.getBlockEntity(heightPos.above(outOfGround));
